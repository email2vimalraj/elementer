...
;(function(document) {
  ...
  function cssPath(el) {
    ...
    while (el) {
      ...
      cssClass = el.getAttribute("class")
    ...
    for (i = 0; i < parentSelectors.length; i++) {
      ...
      if (
        useNthChild &&
        !parentSelectors[i].match(/#/) &&
        !parentSelectors[i].match(/^(html|body)$/)
      ) {
        ...
        if (!parentSelectors[i].match(/\./) || $(cssPathStr).length > 1) {
          ...
          for (
            ...
            c = c.previousElementSibling, nth++
          );
  ...
  function getXpath(el) {
    ...
    if (el.tagName === "BODY") return el.tagName.toLowerCase()
